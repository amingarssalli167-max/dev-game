...
class Col{constructor(c){this.set(c);}
  set(c){this.v=c;return this;} setHex(h){this.v=h;return this;}
  setRGB(r,g,b){this.v=[r,g,b];return this;} getHex(){return 0;}}
class Obj3D{
...
const THREE={
  ...
  PointLight,SpotLight,HemisphereLight,AmbientLight,
...
function buildOutdoor(){
  ...
  s.lights.push({x:22.5,y:22.5,r:7,c:'rgba(255,150,60,',a:.30,flick:.35,fire:true});
  s.lights.push({x:24.6,y:20.4,r:3.4,c:'rgba(255,120,40,',a:.22,flick:.4,fire:true});
  /* checkpoint */
  ...
  s.lights.push({x:31,y:34,r:6,c:'rgba(255,210,170,',a:.16,flick:.25,fire:true});
...
function updatePlayer(dt){
  ...
  /* camera-relative movement: W always walks where you look, never off to the side */
  if(ix||iy){
    const ca=Math.cos(IN.aim),sa=Math.sin(IN.aim);
    const sx=ix,sy=iy;                 /* screen space: +x right, +y down  */
    ix=sx*ca+sy*sa;                    /* -> world east  (forward = aim)   */
    iy=-sx*sa+sy*ca;                   /* -> world south (right   = aim+90)*/
    if(typeof __dbgMove!=='undefined')__dbgMove({sx:sx,sy:sy,ca:ca,sa:sa,aim:IN.aim,ix:ix,iy:iy,
      t1:sx*ca,t2:sy*sa,t3:-sx*sa,t4:sy*ca,sum1:sx*ca+sy*sa,sum2:-sx*sa+sy*ca,typeix:typeof ix});
  }
  ...
  /* vitals display */
  const hot=P.sta<45||P.bat<30||P.rad>18;
...
function renderNb(){
  ...
  if(NB.tab==='obj'){
    ...
    const inv=Object.keys(G.items).filter(k=>!k.startsWith('doc_')&&!k.startsWith('tape'));
...
function bindInput(){
  window.addEventListener('keydown',e=>{
    if(['Tab','F5','Space'].includes(e.code))e.preventDefault();
    if(e.repeat)return;
    IN.down(e);
    ...
    switch(e.code){
      ...
      case 'KeyQ':if(hasItem('medkit')&&G.items.medkit>0){G.items.medkit--;G.player.rad=Math.max(0,G.player.rad-45);
...
function togglePause(){
  ...
  if(G.paused){
    ...
    if(A.ready)A.master.gain.value=0.25;
...
boot();global.__dbgMove=o=>console.log('  dbg',JSON.stringify(o));
boot();startGame(true);
