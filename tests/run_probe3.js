...
boot();console.log('opacity right after parse (empty store):',JSON.stringify($('btnCont').style.opacity));
console.log('hasSave() with empty store:',hasSave());
const fs=require('fs');
const st=JSON.parse(fs.readFileSync('/tmp/ch_reopen_store.json','utf8'));
for(const k in st)__store[k]=st[k];
console.log('opacity after seeding:',JSON.stringify($('btnCont').style.opacity));
boot();
console.log('opacity after boot:',JSON.stringify($('btnCont').style.opacity));
