...
boot();const fs=require('fs');
const st=JSON.parse(fs.readFileSync('/tmp/ch_reopen_store.json','utf8'));
for(const k in st)__store[k]=st[k];
console.log('probe hasSave before boot:',hasSave());
console.log('probe SAVEKEY value len:',(localStorage.getItem(SAVEKEY)||'').length);
console.log('probe slot2 len:',(localStorage.getItem(slotKey('2'))||'').length);
boot();
console.log('probe hasSave after boot:',hasSave());
console.log('btnCont opacity:',$('btnCont').style.opacity,'disabled:',$('btnCont').disabled);
